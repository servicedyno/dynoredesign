...
```bash
curl -X GET "https://repo-env-config.preview.emergentagent.com/api/wallet/getWallet?company_id=1" \
  -H "Authorization: Bearer <YOUR_JWT_TOKEN>"
```

### Method 2: Try Adding Duplicate
```bash
curl -X POST "https://repo-env-config.preview.emergentagent.com/api/wallet/validateWalletAddress" \
  -H "Authorization: Bearer <YOUR_JWT_TOKEN>" \
