backend:
  - target_url: https://crypto-payment-hub-20.preview.emergentagent.com/api
  - test_endpoints:
  ...
  - recent_fixes:
    ...
    - VERIFIED (2026-06-29 11:45 UTC): Telnyx profile switch regression test PASSED ✅ - After switching TELNYX_VERIFY_PROFILE_ID to 49000190-3429-96c2-347f-ba26862735da (repurposed working profile with app_name=DynoPay, code_length=6), the phone onboarding API is working correctly:
      * GET /api/ → HTTP 200 (Health check operational)
      * POST /api/user/registerPhone {"mobile":"18022100479"} → HTTP 503 "Failed to send verification code" (EXPECTED - account's own DID is undeliverable, delivery_status=delivery_failed detected correctly after ~2.8s polling)
      * POST /api/user/registerPhone {} → HTTP 400 "Mobile number is required" (validation working)
      * Backend logs confirm: verification created with profile 49000190-3429-96c2-347f-ba26862735da, delivery status polling detected delivery_failed, clean 503 returned (NOT misleading 200, NOT 500, NOT hang)
      * PASS CRITERIA MET: No HTTP 500 errors, health check operational, registerPhone returns clean 503/200 within ~3s (not hang), missing-mobile validation returns 400
      * NOTE: Actual SMS delivery + DynoPay branding to a real handset requires separate user confirmation (own-DID test only validates the delivery-status detection path)
...
frontend:
  - target_url: https://crypto-payment-hub-20.preview.emergentagent.com
  - recent_fixes:
    - FIX (2026-06-29): next/image "hostname dynopay.com is not configured" runtime error (user avatars served from https://dynopay.com/images/... via the shared prod DB). ROOT CAUSE: next.config.mjs images.remotePatterns only whitelisted api.dynopay.com + **.preview.emergentagent.com, not the apex dynopay.com. FIX: added remotePatterns for "dynopay.com" (apex) and "**.dynopay.com" (subdomains incl. checkout). Frontend restarted (config change). VERIFIED by frontend testing agent: /_next/image for dynopay.com→200, checkout.dynopay.com→200, api.dynopay.com→ok, unconfigured host still 400 (whitelist specific), homepage has no "not configured"/"Invalid src prop" errors.
...
- test_url: https://crypto-payment-hub-20.preview.emergentagent.com
...
- test_url: https://crypto-payment-hub-20.preview.emergentagent.com/auth/onboarding-preview
...
- test_results: MIXED RESULTS ⚠️
  * Target URL https://crypto-payment-hub-20.preview.emergentagent.com/api → HTTP 404 (Service not available at this URL)
  * Current URL https://crypto-payment-hub-20.preview.emergentagent.com/api → ALL TESTS PASSED ✅
...
- target_url: https://crypto-payment-hub-20.preview.emergentagent.com/documentation
...
1. ✅ Login Page Load Test
   - URL: https://crypto-payment-hub-20.preview.emergentagent.com/auth/login
...
3. ✅ Landing Page Load Test
   - URL: https://crypto-payment-hub-20.preview.emergentagent.com/
...
- test_url: https://crypto-payment-hub-20.preview.emergentagent.com/auth/register
