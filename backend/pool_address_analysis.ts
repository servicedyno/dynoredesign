...
async function runPoolAddressAnalysis() {
  try {
    ...
    if (poolAddresses.length === 0) {
      ...
      console.log('2. User ID 28 is incorrect');
      console.log('3. Pool system has not been initialized for this user');
      return;
    }
    ...
    // Final conclusion
    console.log(`\n🎯 CONCLUSION:`);
    if (bulkCreation && merchantCorrelation && transactionCorrelation) {
      ...
      console.log('   Evidence: All addresses created within minutes, correlated with merchant wallet setup,');
      console.log('   and transactions came after address creation.');
