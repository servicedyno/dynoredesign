...
// Diagnostics: Recovery for failed payments — resets Redis state and re-runs cryptoVerification
app.post("/diagnostics/recover-payment", authMiddleware, async (req: express.Request, res: express.Response) => {
  try {
    ...
    log(`Admin triggered payment recovery for address: ${address}`, "info");
    
    // Read current Redis state
    const redisKey = `crypto-${address}`;
    const currentData = await getRedisItem(redisKey);
    
    if (!currentData || Object.keys(currentData).length === 0) {
      return res.status(404).json({ success: false, error: "No payment data found in Redis for this address" });
    }
    
    if (currentData.status === "successful" || currentData.status === "completed") {
      return res.status(400).json({ success: false, error: `Payment already ${currentData.status}`, data: currentData });
    }
    
    log(`[Recovery] Current state: status=${currentData.status}, txId=${currentData.txId}, receivedAmount=${currentData.receivedAmount}`, "info");
    
    // Reset to processing state (keeps txId and receivedAmount)
    ...
    log(`[Recovery] Redis state reset to 'processing', calling cryptoVerification...`, "info");
    
    // Run cryptoVerification
    const result = await paymentController.cryptoVerification(address, true, redisKey);
    
    if (result && result.status && result.status >= 400) {
      // Restore failed state with new error
      await setRedisItem(redisKey, {
        ...currentData,
        status: "failed",
      ...
      return res.status(result.status).json({ success: false, error: result.message, result });
    }
    ...
    log(`[Recovery] ✅ Payment recovered successfully for address: ${address}`, "info");
    res.status(200).json({ success: true, message: "Payment recovered successfully", result });
    
  } catch (error) {
    log(`[Recovery] ❌ Error: ${getErrorMessage(error)}`, "error");
    res.status(500).json({ success: false, error: getErrorMessage(error) });
  }
});
