#!/usr/bin/env python3
"""
DynoPay Device Login Email Testing Suite - Focused Test
...
class DeviceLoginEmailTester:
    ...
    def check_email_template_location(self):
        ...
        try:
            ...
            if result.returncode == 0 and "Location:" in result.stdout:
                # Check if it's in the sendNewDeviceLoginEmail context
                if "locationDisplay" in result.stdout:
                    self.log_result(
                        ...
                        "Location information found prominently displayed as first item in login details",
                        {
                            ...
                            "context": "sendNewDeviceLoginEmail function"
                        }
                    )
                else:
                    self.log_result(
                        "Email Template Location Display", 
                        True, 
                        "Location information found in email template",
                        {"grep_output": result.stdout[:200]}
                    )
            else:
                self.log_result(
                    "Email Template Location Display", 
    ...
    def login_user(self, attempt_number: int):
        """Login with john@dyno.pt credentials using different IPs"""
        ...
        try:
            ...
            print(f"Using IP: {ip_addresses[attempt_number - 1]}")
            ...
            if response.status_code == 200:
                ...
                if 'data' in data and 'accessToken' in data['data']:
                    ...
                    self.log_result(
                        ...
                        True, 
                        f"Successfully authenticated {user_data.get('email', 'user')} from IP {ip_addresses[attempt_number - 1]}",
                        {
                            "user_id": user_data.get('user_id'),
                            "name": user_data.get('name'),
                            "email": user_data.get('email'),
                            "ip_used": ip_addresses[attempt_number - 1],
                            "timestamp": time.time()
                        }
                    )
                    return True
    ...
    def test_rapid_login_sequence(self):
        ...
        # Perform first login (should trigger new device alert)
        login1_success = self.login_user(1)
        
        # Wait a brief moment (less than cache timeout)
        time.sleep(3)
        
        # Perform second login immediately (should NOT trigger duplicate alert due to Redis cache)
        ...
        if login1_success and login2_success:
            self.log_result(
                ...
                {
                    "login_count": 2,
                    "time_between_logins": "~3 seconds",
                    "expected_behavior": "First login should trigger device alert, second should be cached"
                }
            )
        else:
            self.log_result(
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                for line in log_content.split('\n'):
                    ...
                    if 'New device alert sent to' in line and (',' in line or 'Unknown location' in line):
                ...
                if alert_count == 1:
                    self.log_result(
                        ...
                        True, 
                        f"Found exactly 1 device alert message (Redis cache working correctly)",
                        {
                    ...
                    self.log_result(
                        "Redis Duplicate Prevention", 
                        False, 
                        "No device alert messages found in logs - may need different IP or user state",
                        {"note": "Device alerts only trigger when IP changes from last_login_ip"}
                    )
                elif alert_count == 2:
                    self.log_result(
                        "Redis Duplicate Prevention", 
                    ...
                    self.log_result(
                        "Redis Duplicate Prevention", 
                        True, 
                        f"Found {alert_count} device alert messages - analyzing pattern",
                        {
                            "alert_count": alert_count,
                            "alert_messages": device_alert_lines
                        }
                ...
                elif device_alert_lines:
                    # Check if device alerts exist but without clear geolocation format
                    has_location_info = any('(' in line and ')' in line for line in device_alert_lines)
                    if has_location_info:
                        self.log_result(
                            "IP Geolocation in Logs", 
                            True, 
                            "Device alerts found with location information in parentheses",
                            {
                                "device_alerts": device_alert_lines,
                                "note": "Location info appears to be included in alert messages"
                            }
                        )
                    else:
                        self.log_result(
                            "IP Geolocation in Logs", 
                            False, 
                            "Device alerts found but no clear geolocation information detected",
                            {
                                "device_alerts": device_alert_lines,
    ...
    def run_all_tests(self):
        ...
        self.check_email_template_location()
    ...
    def print_summary(self):
        ...
        print(f"\n🎯 EXPECTED BEHAVIOR:")
        print(f"  - First login from new IP: Triggers new device alert email")
        print(f"  - Second login from different IP (within 5min cooldown): No duplicate email sent")
