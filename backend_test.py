...
class ApiTester:
    ...
    def test_merchant_api_endpoints(self, swagger_spec: Dict[str, Any]) -> bool:
        ...
        if not missing_endpoints:
            ...
            # Provide additional context about what exists instead
            existing_alternatives = {
                "/api/user/createPayment": "/api/user/cryptoPayment",
                "/api/user/addFunds": "/api/user/getBalance", 
                "/api/user/useWallet": "/api/user/getSupportedCurrency",
                "/api/user/getSingleTransaction/{id}": "/api/user/getTransactions",
                "/api/user/getCryptoTransaction/{address}": "/api/user/getTransactions"
            }
            
            alternatives_found = []
            for missing in missing_endpoints:
                alt = existing_alternatives.get(missing)
                if alt and alt in paths:
                    alternatives_found.append(f"{missing}→{alt}")
            
            message = f"Missing: {', '.join(missing_endpoints)}"
            if alternatives_found:
                message += f" (alternatives exist: {', '.join(alternatives_found)})"
            
            self.log(message, test_name, "FAIL")
            return False
