...
### Check Swagger UI:
1. Visit: `https://trustline-install.preview.emergentagent.com/api/docs`
...
```bash
curl -X GET "https://trustline-install.preview.emergentagent.com/api/wallet/getWallet?company_id=38" \
...
```bash
curl -X GET "https://trustline-install.preview.emergentagent.com/api/wallet/getWalletAddresses?company_id=38" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
