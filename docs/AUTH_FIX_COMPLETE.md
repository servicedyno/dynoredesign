...
```bash
curl -X POST "https://init-chain.preview.emergentagent.com/api/user/registerUser" \
...
```bash
curl -X POST "https://init-chain.preview.emergentagent.com/api/user/login" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "your@email.com",
    "password": "YourPassword123"
...
curl -X POST "https://init-chain.preview.emergentagent.com/api/company/addCompany" \
...
https://init-chain.preview.emergentagent.com/api/docs
