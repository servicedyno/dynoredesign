...
```bash
curl -X POST "https://init-config.preview.emergentagent.com/api/user/registerUser" \
...
```bash
curl -X POST "https://init-config.preview.emergentagent.com/api/user/login" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "your@email.com",
    "password": "YourPassword123"
...
1. **Open Swagger**: https://init-config.preview.emergentagent.com/api/docs
...
curl -X POST "https://init-config.preview.emergentagent.com/api/company/addCompany" \
...
https://init-config.preview.emergentagent.com/api/docs
