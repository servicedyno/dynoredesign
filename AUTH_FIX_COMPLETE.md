...
```bash
curl -X POST "https://repo-env-config.preview.emergentagent.com/api/user/registerUser" \
...
```bash
curl -X POST "https://repo-env-config.preview.emergentagent.com/api/user/login" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "your@email.com",
    "password": "YourPassword123"
...
1. **Open Swagger**: https://repo-env-config.preview.emergentagent.com/api/docs
